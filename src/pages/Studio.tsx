...
const Studio = () => {
  usePageSeo({
    title: "Studio",
    description:
      "Paperfrogs HQ is an infrastructure-first research studio. We build durable systems through clear interfaces and secure-by-default architecture.",
    path: "/studio",
  });
      ...
      <section className="mx-auto w-full max-w-7xl px-6 pt-16 pb-0 sm:px-10 lg:px-16 sm:pt-20">
        <Reveal>
          ...
          <p className="mt-5 max-w-2xl text-base leading-relaxed text-foreground/40">
            Paperfrogs HQ is a small, focused research studio. We combine deep
            technical exploration with production-grade execution — no
            slideware, no speculation, no throwaway prototypes.
          </p>
        </Reveal>
        ...
        <Reveal delay={0.08}>
          <div className="mt-10 flex flex-wrap gap-10 border-t border-white/[0.07] pt-10">
            {[
              ...
              {
                num: String(activeCount).padStart(2, "0"),
                label: "In production",
              },
              {
                num: String(researchCount).padStart(2, "0"),
                label: "In research",
              },
              { num: "Dhaka", label: "Base" },
            ].map(({ num, label }) => (
              <div key={label}>
                <p className="text-2xl font-bold tracking-[-0.025em] text-foreground sm:text-3xl">
                  {num}
                </p>
          ...
          {studioPrinciples.map((principle, i) => (
            <Reveal key={principle} delay={i * 0.04}>
              <div className="flex items-center gap-10 py-7">
                ...
                <p className="text-lg font-bold tracking-[-0.02em] text-foreground/65">
                  {principle}
                </p>
              </div>
            </Reveal>
